...
-- select workspace
CREATE OR REPLACE STREAMING TABLE main.dbsql_warehouse_advisor.workspaces
SCHEDULE EVERY 1 HOUR
CLUSTER BY (workspace_full_name)
AS (
      SELECT
      account_id,
      workspace_id, 
      ...
      FROM STREAM system.access.workspaces_latest WITH (SKIPCHANGECOMMITS)
);


CREATE OR REPLACE STREAMING TABLE main.dbsql_warehouse_advisor.warehouses
SCHEDULE EVERY 1 HOUR
CLUSTER BY (warehouse_id)
AS (
SELECT *,
CONCAT(warehouse_name, ' (id:', warehouse_id, ')') AS warehouse_full_name,
CASE WHEN delete_time IS NOT NULL THEN 'Deleted' ELSE 'Active' END AS is_deleted
FROM STREAM system.compute.warehouses WITH (SKIPCHANGECOMMITS)
);


CREATE OR REPLACE STREAMING TABLE main.dbsql_warehouse_advisor.warehouse_query_history
SCHEDULE EVERY 1 HOUR
CLUSTER BY (start_date, workspace_id, warehouse_id)
...
FROM STREAM system.query.history WITH (SKIPCHANGECOMMITS)
...
CREATE OR REPLACE STREAMING TABLE main.dbsql_warehouse_advisor.warehouse_usage
SCHEDULE EVERY 1 HOUR
CLUSTER BY (usage_date, workspace_id, warehouse_id)
COMMENT 'SQL Warehouse Usage'
AS 
SELECT 
usage_metadata.warehouse_id AS warehouse_id,
*,
usage_start_time::date AS usage_start_date
FROM STREAM system.billing.usage WITH (SKIPCHANGECOMMITS)
WHERE usage_metadata.warehouse_id IS NOT NULL;



-- Warehouse Scaling History
CREATE OR REPLACE STREAMING TABLE main.dbsql_warehouse_advisor.warehouse_scaling_events
SCHEDULE EVERY 1 HOUR
CLUSTER BY (workspace_id, warehouse_id, event_date)
COMMENT 'SQL Warehouse Scaling Events from warehouse_events table'
AS
SELECT *,
event_time::date AS event_date
 FROM STREAM system.compute.warehouse_events WITH (SKIPCHANGECOMMITS);
...
CREATE OR REPLACE STREAMING TABLE main.dbsql_warehouse_advisor.warehouse_raw_events
SCHEDULE EVERY 1 HOUR
CLUSTER BY (workspace_id, warehouse_id, event_date)
AS 
    SELECT 
    ...
    FROM STREAM system.access.audit WITH (SKIPCHANGECOMMITS)
